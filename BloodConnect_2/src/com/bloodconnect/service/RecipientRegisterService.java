...
public boolean register(Recipient recipient){
	 ...
	 if(isUserExists(recipient)) return false;	
...
public boolean isUserExists(Recipient recipient){
	 ...
	 try{
		 tx = session.getTransaction();
		 tx.begin();
		 Query query = session.createQuery("from Recipient where RecipientUserId='"+recipient.getRecipientUserId()+"'");
		 Recipient u = (Recipient)query.uniqueResult();
		 tx.commit();
